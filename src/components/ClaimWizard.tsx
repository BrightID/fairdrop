...
const ClaimWizard = ({
    ...
    switch (stepState) {
      case TxStates.WaitingSignature:
        if (claimState.txState <= stepState) {
          return (
            <Typography
              className={classNames.stepContentContainer}
              variant={'h5'}
            >
  ...
  const buildStepItem = (stepState: TxState) => {
    ...
    return (
      <Grid container className={classNames.stepContainer}>
  ...
  const buildDone = () => {
    return (
      <Box textAlign={'center'} className={classNames.doneContainer}>
  ...
  const buildError = () => {
    return (
      ...
      >
        <AlertTitle>
          <Typography variant={'h6'}>Claiming failed</Typography>
        </AlertTitle>
  ...
  return (
    ...
    >
      <Box className={classNames.dialog}>
        ...
        <DialogContent>{buildContent()}</DialogContent>
      </Box>
    </Dialog>
  );
};
