...
const HashDisplay = ({hash, type, className, withEtherscanLink, chainId}: HashDisplayProps) => {
    ...
    if (hash) {
        return <>
            <Tooltip title={hash} interactive classes={{tooltip: classes.toolTip}}>
                <span className={className}>{hash.substring(0, 7)}...{hash.substring(hash.length - 4)}</span>
            </Tooltip>
            {withEtherscanLink &&
            <IconButton color={'inherit'} size={'small'} href={buildLink()} target={'_blank'}>
              <OpenInNewIcon/>
            </IconButton>
            }
        </>
...
const useStyles = makeStyles((theme: Theme) => createStyles({
    toolTip: {
        maxWidth: 'none', borderRadius: '4px'
    },
}),)
